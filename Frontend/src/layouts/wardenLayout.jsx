...
import {
  ...
  Avatar, Divider, IconButton, Tooltip, Chip, Badge,
} from "@mui/material";
import {
  Dashboard, People, MeetingRoom,
  AdminPanelSettings, Logout, Assessment,
  KeyboardArrowRight, NotificationsOutlined, AccountBalanceWallet,
} from "@mui/icons-material";
import { useState, useEffect } from "react";

const drawerWidth = 260;

const SIDEBAR_BG = "linear-gradient(180deg, #1E3A8A 0%, #1e40af 60%, #2563eb 100%)";
const ACTIVE_BG  = "rgba(255,255,255,0.15)";
const HOVER_BG   = "rgba(255,255,255,0.08)";

const navItems = [
  { label: "Dashboard", icon: <Dashboard fontSize="small" />,    path: "/warden" },
  { label: "Students",  icon: <People fontSize="small" />,       path: "/warden/students" },
  { label: "Rooms",     icon: <MeetingRoom fontSize="small" />,  path: "/warden/rooms" },
  { label: "Payments",  icon: <AccountBalanceWallet fontSize="small" />, path: "/warden/payments" },
  { label: "Reports",   icon: <Assessment fontSize="small" />,   path: "/warden/reports" },
];
...
const WardenLayout = () => {
  ...
  const displayName = user.username || "Warden";
      ...
      >
        ...
        <Box sx={{ px: 3, py: 3, display: "flex", alignItems: "center", gap: 1.5, borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          <Box>
            ...
            <Typography fontSize={11} sx={{ color: "rgba(255,255,255,0.6)", fontWeight: 500 }}>Warden Panel</Typography>
          </Box>
        </Box>
        ...
        <Box sx={{ mx: 2, my: 2, p: 2, borderRadius: 2, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)", display: "flex", alignItems: "center", gap: 1.5 }}>
          ...
          <Box sx={{ overflow: "hidden" }}>
            ...
            <Chip label="Warden" size="small" sx={{ height: 18, fontSize: 10, fontWeight: 600, backgroundColor: "rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.9)", border: "1px solid rgba(255,255,255,0.2)" }} />
          </Box>
        </Box>
          ...
          <Toolbar sx={{ justifyContent: "space-between", minHeight: "64px !important", px: { xs: 2, sm: 3 } }}>
            ...
            <Box>
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                ...
                <Typography fontSize={11} color="primary.main" fontWeight={600}>{currentPage}</Typography>
              </Box>
            </Box>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              ...
              <Avatar sx={{ background: "linear-gradient(135deg, #1E3A8A, #3B82F6)", color: "#fff", width: 34, height: 34, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                {displayName[0]?.toUpperCase()}
              </Avatar>
